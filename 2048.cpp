...
const int scr_width = 120;
const int scr_height = 30;
...
bool TGame::CheckEndGame()
{
	...
	for (int i=0; i<fld_width; i++)
	{
		...
		{
			if ( (j<fld_height-1 && cell[j][i].value==cell[j+1][i].value) || (i<fld_width-1 && cell[j][i].value==cell[j][i+1].value) )
			return false;
		}
	}
	return true;
...
void TGame::Move(int dx, int dy)
{
	...
	{
		cout<<"You lose!";
		cout<<"Game will automatically restart after 10 seconds.";
		Sleep(10);
		Init();
	}
	else
